...
struct NTT {
  ...
  explicit NTT(u32 mod = 998244353) : mt(mod), mts(mod), mod(mod) {
    const auto mt = this->mt;  // ! to put Montgomery constants in registers
  ...
  // writes (a * b) to a
  [[gnu::noinline]] __attribute__((optimize("O3"))) void conv(int lg, __restrict__ pu32 a, __restrict__ pu32 b) const {
    if (lg <= 4) {
      int n = (1 << lg);
      __restrict__ pu32 c = (pu32)_mm_malloc(n * 4, 4);
      memset(c, 0, 4 * n);
      for (int i = 0; i < n; ++i)
        for (int j = 0; j < n; ++j) c[(i + j) & (n - 1)] = mt.shrink2(c[(i + j) & (n - 1)] + mt.mul(a[i], b[j]));
      for (int i = 0; i < n; ++i) a[i] = mt.mul<true>(mt.r2, c[i]);
      _mm_free(c);
      return;
    }
    fft(lg, a);
  ...
  __attribute__((optimize("O3"))) vec<u32> conv(const vec<u32> &a, const vec<u32> &b) const {
    ...
    pu32 ap = (pu32)_mm_malloc(std::max(64, (1 << lg) * 4), 64);
    pu32 bp = (pu32)_mm_malloc(std::max(64, (1 << lg) * 4), 64);
    memset(ap, 0, 4 << lg);
    memset(bp, 0, 4 << lg);
