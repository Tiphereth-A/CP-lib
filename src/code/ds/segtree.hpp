...
class segtree {
  struct YYZ {
    ...
    friend YYZ operator%(YYZ a, const T &mod) {
      a.w %= mod, a._min %= mod, a._max %= mod;
      a.sign %= mod, a.sign1 %= mod;
      return a;
    }
    friend YYZ operator%=(YYZ &a, const T &mod) {
      return a = a % mod;
 ...
 public:
  ...
  explicit constexpr segtree(vec<T> const &a, T MOD = INT64_MAX) : t(a.size() * 4), mod(MOD) { build(a, 1, 0, a.size() - 1); }
};
