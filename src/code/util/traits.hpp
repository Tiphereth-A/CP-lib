...
template <class T> concept istream_c = requires(T is, T& (*func)(T&)) { { is >> func } -> std::same_as<T&>; };
template <class T> concept ostream_c = requires(T os, T& (*func)(T&)) { { os << func } -> std::same_as<T&>; };
