...
inline poly<T> gen_bernoulli(u32 n, vec<mint> const& fact, vec<mint> const& ifact) {
...
// bernoulli[i] = B_i, i=0,1,...,n
template <class T, class mint = typename T::value_type>
inline poly<T> gen_bernoulli(u32 n) { return gen_bernoulli<T, mint>(n, gen_fact<mint>(n + 1), gen_ifact<mint>(n + 2)); }
