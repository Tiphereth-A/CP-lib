...
struct ConvexHull: public Polygon {
  ConvexHull() = default;
  explicit ConvexHull(size_t size_):
    Polygon(size_) {}
  explicit ConvexHull(const cont_t<Point> &vs_, bool inited = false, bool strict = true):
    Polygon(vs_) {
  ...
  TIFA_LIBS_GEO2D_FNUC_iSELF1__(ConvexHull, minkowski_sum_nonstrict, const ConvexHull &, rhs, {
  ...
  TIFA_LIBS_GEO2D_FNUC_iSELF1__(ConvexHull, minkowski_sum, const ConvexHull &, rhs, { return do_minkowski_sum_nonstrict(rhs).init(); });
  TIFA_LIBS_GEO2D_FNUC_iSELF1__(ConvexHull, ins_CVHhP, const Line &, l, {
    size_t sz = vs.size();
    cont_t<Point> cvc;
