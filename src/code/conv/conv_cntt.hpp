...
template <class mint>
vec<mint> conv_cntt(vec<mint> const &l, vec<mint> const &r) { return conv_cntt(l, r, u32(l.size() + r.size() - 1)); }
...
template <class mint, class T = u64>
vec<mint> conv_cntt(vec<T> const &l, vec<T> const &r) { return conv_cntt<mint, T>(l, r, u32(l.size() + r.size() - 1)); }
