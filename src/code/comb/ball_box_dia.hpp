...
template <class T>
constexpr auto ball_box_dia(u32 ball, u32 box, poly<T> const& s2r) { return std::reduce(s2r.data().begin(), s2r.data().begin() + (std::min(ball, box) + 1), typename poly<T>::value_type{}); }
template <class T>
