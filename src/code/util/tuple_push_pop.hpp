...
CEXP auto subtuple_impl_(Tpl &&t, std::index_sequence<Is...>) { return std::make_tuple(std::get<Is + bgn>(t)...); }
}  // namespace tuple_push_pop_impl_
...
CEXP auto tuple_push(T &&v, Tpl &&t) {
...
CEXP auto tuple_push_front(T &&v, Tpl &&t) { return tuple_push<0>(v, t); }
template <class T, class Tpl>
CEXP auto tuple_push_back(T &&v, Tpl &&t) { return tuple_push<tuple_size<Tpl>>(v, t); }

template <usz pos, class Tpl>
CEXP auto tuple_pop(Tpl &&t) {
  static_assert(tuple_size<Tpl> > 0, "Pop from empty tuple");
  return std::tuple_cat(subtuple<0, pos>(t), subtuple<pos + 1, tuple_size<Tpl> - pos - 1>(t));
}
template <class Tpl>
CEXP auto tuple_pop_front(Tpl &&t) { return tuple_pop<0>(t); }
template <class Tpl>
CEXP auto tuple_pop_back(Tpl &&t) { return tuple_pop<tuple_size<Tpl> - 1>(t); }
