...
from libs.content.typst_utils import PathTypst, typst_listing_code_range
...
class Section:
    ...
    def _validate_doc_consistency(self, src_list: set[str]) -> tuple[bool, bool]:
        has_tex = "doc.tex" in src_list
        has_typ = "doc.typ" in src_list
        if has_tex != has_typ:
            raise ValueError(
                f"Section '{self._name}': 'doc.tex' and 'doc.typ' must both exist or neither")
        return has_tex, has_typ

    @with_logger
    def expand_tex(self, temp_path: str, **kwargs):
        src_list = self._get_src_list()
        has_tex, has_typ = self._validate_doc_consistency(src_list)
        custom_doc = has_tex
        if has_tex:
            src_list.remove("doc.tex")
        if has_typ:
            src_list.remove("doc.typ")
    ...
    def expand_typ(self, temp_path: str, **kwargs):
        ...
        result_path = os.path.join(temp_path, self._dir, "doc.typ")
        os.makedirs(os.path.dirname(result_path), exist_ok=True)
        with open(result_path, 'w', encoding='utf8') as f_result:
            content: str = _TEMPLACES["doc.typ"] if not custom_doc else open(
                os.path.join(self._dir, "doc.typ"), 'r', encoding='utf8').read()
            for file in src_list:
                pattern = re.compile(
                    r'// \{' + re.escape(file) + r'(?:,start=(-?\d+))?(?:,stop=(-?\d+))?\}')
                total_lines: int = 0
                ...
                while True:
                    ...
                    content = content.replace(
                        match.group(0),
                        ''.join(typst_listing_code_range(
                            PathTypst(os.path.join(self._dir, file)),
                            _EXT_TYPE[os.path.splitext(file)[1]],
                            start,
                            stop)))
            f_result.write(content)
