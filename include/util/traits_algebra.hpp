...
struct properties_optt {  // $\forall$ omitted
...
struct properties_optt_op2tt {  // $\forall$ omitted
  ...
  MERGE_(distributivity, properties_optt<T, op2>::commutativity);
  ...
  MERGE_(absorption, properties_optt<T, op>::commutativity && properties_optt<T, op2>::commutativity);
};
#undef MERGE_
template <class T, class R, auto mapping> requires mapping_c<T, R, mapping>
struct properties_mp {  // $\forall$ omitted
...
struct properties_optt_mp {  // $\forall$ omitted
...
struct properties_oprr_mp {  // $\forall$ omitted
...
struct properties_optt_op2rr_mp {  // $\forall$ omitted
...
template <class T, auto op> concept semigroup_c = !CHECK_ALGEBRA_ || (magma_c<T, op> && properties_optt<T, op>::associativity);
template <class T, auto op> concept quasigroup_c = !CHECK_ALGEBRA_ || (magma_c<T, op> && properties_optt<T, op>::divisibility);
template <class T, auto op> concept unital_magma_c = !CHECK_ALGEBRA_ || (magma_c<T, op> && properties_optt<T, op>::identity);
template <class T, auto op> concept monoid_c = !CHECK_ALGEBRA_ || (semigroup_c<T, op> && properties_optt<T, op>::identity);
template <class T, auto op> concept loop_c = !CHECK_ALGEBRA_ || (unital_magma_c<T, op> && properties_optt<T, op>::divisibility);
template <class T, auto op> concept associative_quasigroup_c = !CHECK_ALGEBRA_ || (quasigroup_c<T, op> && properties_optt<T, op>::associativity);
template <class T, auto op> concept group_c = !CHECK_ALGEBRA_ || (semigroup_c<T, op> && properties_optt<T, op>::divisibility);
template <class T, auto op> concept abelian_group_c = !CHECK_ALGEBRA_ || (group_c<T, op> && properties_optt<T, op>::commutativity);
// lattice-like
template <class T, auto op> concept semilattice_c = !CHECK_ALGEBRA_ || (magma_c<T, op> && properties_optt<T, op>::associativity && properties_optt<T, op>::commutativity && properties_optt<T, op>::idempotency);
template <class T, auto join, auto meet> concept lattice_c = !CHECK_ALGEBRA_ || (semilattice_c<T, join> && semilattice_c<T, meet> && properties_optt_op2tt<T, join, meet>::absorption);
template <class T, auto join, auto meet> concept distributive_lattice_c = !CHECK_ALGEBRA_ || (lattice_c<T, join, meet> && properties_optt_op2tt<T, join, meet>::distributivity && properties_optt_op2tt<T, meet, join>::distributivity);
// ring-like
template <class T, auto add, auto mul> concept left_nearring_c = !CHECK_ALGEBRA_ || (group_c<T, add> && semigroup_c<T, mul> && properties_optt_op2tt<T, add, mul>::left_distributivity);
template <class T, auto add, auto mul> concept right_nearring_c = !CHECK_ALGEBRA_ || (group_c<T, add> && semigroup_c<T, mul> && properties_optt_op2tt<T, add, mul>::right_distributivity);
...
template <class T, auto add, auto mul> concept semiring_c = !CHECK_ALGEBRA_ || (monoid_c<T, add> && monoid_c<T, mul> && properties_optt<T, add>::commutativity && properties_optt<T, mul>::nil && properties_optt_op2tt<T, add, mul>::distributivity);
template <class T, auto add, auto mul> concept ring_c = !CHECK_ALGEBRA_ || (rng_c<T, add, mul> && semiring_c<T, add, mul>);
template <class T, auto add, auto mul> concept commutative_ring_c = !CHECK_ALGEBRA_ || (ring_c<T, add, mul> && properties_optt<T, mul>::commutativity);
template <class T, auto add, auto mul> concept domain_c = !CHECK_ALGEBRA_ || (ring_c<T, add, mul> && properties_optt<T, mul>::non_zero_divisor);
...
template <class T, auto add, auto mul> concept division_ring_c = !CHECK_ALGEBRA_ || (ring_c<T, add, mul> && properties_optt<T, mul>::invertiblity);
template <class T, auto add, auto mul> concept field_c = !CHECK_ALGEBRA_ || (integral_domain_c<T, add, mul> && abelian_group_c<T, mul>);
// module-like
template <class T, class R, auto tadd, auto radd, auto rmul, auto mapping> concept Rmodule_c = !CHECK_ALGEBRA_ || (abelian_group_c<T, tadd> && ring_c<R, radd, rmul> && properties_optt_mp<T, R, tadd, mapping>::distributivity && properties_oprr_mp<T, R, radd, mapping>::associativity && properties_oprr_mp<T, R, rmul, mapping>::associativity && properties_mp<T, R, mapping>::identity);
