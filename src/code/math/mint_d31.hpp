...
class mint_d31 {
 ...
 protected:
  ...
  raw_t v_;
  CEXP mint_d31() : v_{} {}
  CEXP mint_d31(int_c auto v) : v_{mod(v)} {}
  template <int_c T>
  static CEXP raw_t mod(T v) { return tsf(norm(i32(v % (std::conditional_t<sint_c<T>, i32, u32>)mod()))); }
  ...
  CEXP auto neg() const {
    mint res;
    raw_t h = v_ >> OFFSET;
    return res.v_ = (((MOD_ODD & -(h != 0)) - h) << OFFSET) | (-v_ & MASK), res;
  }
  CEXP void add(mint_d31 CR r) {
    raw_t h = (v_ >> OFFSET) + (r.v_ >> OFFSET) - MOD_ODD;
    v_ = ((h + (MOD_ODD & -(h >> 31))) << OFFSET) | ((v_ + r.v_) & MASK);
  }
  CEXP void sub(mint_d31 CR r) {
    raw_t h = (v_ >> OFFSET) - (r.v_ >> OFFSET);
    v_ = ((h + (MOD_ODD & -(h >> 31))) << OFFSET) | ((v_ - r.v_) & MASK);
  }
  CEXP void mul(mint_d31 CR r) { v_ = (redc((u64)(v_ >> OFFSET) * (r.v_ >> OFFSET)) << OFFSET) | ((v_ * r.v_) & MASK); }
};
