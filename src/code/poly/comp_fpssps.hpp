...
// @return $f(g(x_0, \dots, x_{n-1}))$
template <class mint, class ccore, u32 N = 21>
CEXP auto comp_fpssps(u32 n, poly<mint, ccore> CR f, vec<mint> CR g) { return comp_fpssps<mint, ccore, N>(n, f, g, gen_fact(max((u32)f.size(), n) + 1, mint::mod()), gen_ifact(max((u32)f.size(), n) + 1, mint::mod())); }
