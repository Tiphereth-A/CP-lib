...
CEXP void level_order(pointer p, F &&f) {
  if (!p) return;
  u32 level = 0;
...
CEXP void dealloc_subtree(pointer p, Alloc &alloc) {
  if (!p) return;
...
CEXP pointer most(pointer p, bool dir) {
  if (!p) return nullptr;
  while (p->ch[dir]) p = p->ch[dir];
  return p;
}
template <tp2_ds_c pointer>
CEXP auto leftmost(pointer p) { return most(p, 0); }
template <tp2_ds_c pointer>
CEXP auto rightmost(pointer p) { return most(p, 1); }
