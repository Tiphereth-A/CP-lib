...
// 0: neg=false, dt={}
struct mpi {
  static constexpr int D = 1000000000, logD = 9;
  bool neg;
  vec<i32> dt;
 ...
 private:
  ...
  static std::string itos_(u32 x, bool zero_padding) {
    assert(x < (u32)D);
    std::string res;
    for (int i = 0; i < logD; ++i) res.push_back(char(48 + x % 10)), x /= 10;
    if (!zero_padding) {
  ...
  static i128 to_i128_(vec<i32> const& a) {
    i128 res = 0;
    for (size_t i = a.size() - 1; ~i; --i) res = res * D + a[i];
    return res;
  }
