...
template <int D>
constexpr u32 ch1idx_(u32 i) { return i * D + 1; }
template <int D>
constexpr u32 chls_idx_(u32 i) { return i * D + D; }
template <int D>
constexpr u32 gchls_idx_(u32 i) { return i * (D * D) + (D * D + D); }
template <int D>
constexpr u32 pidx_(u32 i) { return (i - 1) / D; }
template <int D>
constexpr u32 gpidx_(u32 i) { return (i - (D + 1)) / (D * D); }
template <int D>
constexpr u32 idx_ngch_(u32 l) { return gpidx_<D>(l - 1) + 1; }
template <int D, class I, class C>
constexpr I l_ch_(I it_ch1, C &&c) {
  if constexpr (D == 1) return it_ch1;
  else if constexpr (D == 2) return it_ch1 + !!c(it_ch1[0], it_ch1[1]);
  else {
...
template <int D, class I, class C>
constexpr I l_ch_(I it_ch1, int num_ch, C &&c) {
  if constexpr (D == 2) return it_ch1;
  else if constexpr (D == 3) {
    if (num_ch == 1) return it_ch1;
    else return it_ch1 + !!c(it_ch1[0], it_ch1[1]);
...
constexpr void make_dary_heap(I begin, I end, C &&comp = C{}) {
  ...
  if (int num_ch_end = (len - 1) % D; num_ch_end) {
    if (I chl = d_ary_heap_impl_::l_ch_<D>(begin + d_ary_heap_impl_::ch1idx_<D>(idx), num_ch_end, comp); comp(begin[idx], *chl)) std::swap(begin[idx], *chl);
    if (idx == 0) return;
    --idx;
  }
  if (idx > 0) {
    ...
    for (;;) {
      if (I ch = d_ary_heap_impl_::l_ch_<D>(begin + d_ary_heap_impl_::ch1idx_<D>(idx), comp); comp(begin[idx], *ch)) std::swap(begin[idx], *ch);
      if (idx-- == lidx_ngcd) break;
    }
  }
  for (;; --idx) {
    typename std::iterator_traits<I>::value_type value = std::move(begin[idx]);
    u32 mdidx = idx;
    for (;;) {
