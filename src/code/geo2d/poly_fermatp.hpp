...
point<FP> fermatp(polygon<FP> const &poly, Re &engine, const FP begin = 1e10, const FP end = EPS, const FP delta = .999) {
  static std::uniform_real_distribution<FP> u_angle(0, 2 * PI);
  point<FP> ret = poly.vs.front(), pre = poly.vs.front(), now;
  FP dis_ret{}, dis_pre{}, dis_now;
  for (size_t i = 1; i < poly.vs.size(); ++i) dis_pre += dist_PP(poly.vs[i], ret);
  dis_ret = dis_pre;
  for (FP T = begin; T > end; T *= delta) {
    now = pre + make_P_polar(T, u_angle(engine));
    dis_now = 0;
    for (size_t i = 0; i < poly.vs.size(); ++i) dis_now += dist_PP(poly.vs[i], now);
