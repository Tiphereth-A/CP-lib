...
第一行两个数 $n,m$. 接下来 $m$ 行, 每行 $3$ 个数 $u,v,w$, 表示有一条连接 $u$ 与 $v$ 的无向边, 割断它的代价为 $w$. 接下来这一行有一个整数$Q$, 表示询问次数 接下来 $Q$ 行, 每行两个数 $u,v$, 你需要求出 $u$ 与 $v$ 之间的最小割 **注意：因为数据有误, 给定图的真实点数应该是 $n+1$ 个, 编号为 $0$ 到 $n$**
