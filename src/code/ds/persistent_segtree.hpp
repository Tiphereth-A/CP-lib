...
class persistent_segtree {
  ...
  struct YYZ {
    usz w, ls, rs;
    constexpr YYZ(usz W = 0, usz LS = 0, usz RS = 0) : w(W), ls(LS), rs(RS) {}
  };
  vec<YYZ> t;
  vec<usz> root;

 public:
  constexpr persistent_segtree(vec<usz> const& a, usz N) : n(N), cnt(0), t(a.size() * 24, YYZ()), root(a.size()) {
    build(root[0], 0, n - 1, a[0]);
    for (usz i = 1; i < a.size(); ++i) add_(root[i - 1], root[i], 0, n - 1, a[i]);
  }
  constexpr void add(usz old_x, usz x, usz pos) { add_(root[old_x], root[x], 0, n - 1, pos); }
  constexpr usz kth_min(usz x, usz y, usz k) { return x ? kth_min_(root[x - 1], root[y], 0, n - 1, k) : kth_min_(root[y], 0, n - 1, k); }
  constexpr usz kth_max(usz x, usz y, usz k) { return x ? kth_max_(root[x - 1], root[y], 0, n - 1, k) : kth_max_(root[y], 0, n - 1, k); }
  constexpr usz frequency(usz x, usz y, usz pos) { return x ? frequency_(root[x - 1], root[y], 0, n - 1, pos, pos) : frequency_(root[y], 0, n - 1, pos, pos); }
  constexpr usz frequency(usz x, usz y, usz L, usz R) { return x ? frequency_(root[x - 1], root[y], 0, n - 1, L, R) : frequency_(root[y], 0, n - 1, L, R); }
 ...
 private:
  ...
  constexpr void build(usz& x, usz l, usz r, usz pos) {
    x = (++cnt);
    if (l == r) {
      if (l == pos) ++t[x].w;
      return;
  ...
  constexpr void add_(usz old_x, usz& x, usz l, usz r, usz pos) {
    assert(pos >= l && pos <= r);
    t[x = (++cnt)] = t[old_x];
  ...
  constexpr usz kth_min_(usz x, usz y, usz l, usz r, usz k) {
    if (l == r) return l;
  ...
  constexpr usz kth_max_(usz x, usz y, usz l, usz r, usz k) {
    if (l == r) return l;
  ...
  constexpr usz frequency_(usz x, usz y, usz l, usz r, usz L, usz R) {
    assert(R >= l && L <= r);
