...
class persistent_segtree {
  ...
  struct TIFA {
    u32 w, ls, rs;
    CEXP TIFA(u32 W = 0, u32 LS = 0, u32 RS = 0) : w(W), ls(LS), rs(RS) {}
  };
  vec<TIFA> t;
  vecu root;

 public:
  CEXP persistent_segtree(spnu a, u32 N) : n(N), cnt(0), t(a.size() * 24, TIFA()), root(a.size()) {
  ...
  CEXP u32 kth_max(u32 x, u32 y, u32 k) { return x ? kth_max_(root[x - 1], root[y], 0, n - 1, k) : kth_max_(root[y], 0, n - 1, k); }
  CEXP u32 frequency(u32 x, u32 y, u32 pos) { return x ? frequency_(root[x - 1], root[y], 0, n - 1, pos, pos) : frequency_(root[y], 0, n - 1, pos, pos); }
  CEXP u32 frequency(u32 x, u32 y, u32 L, u32 R) { return x ? frequency_(root[x - 1], root[y], 0, n - 1, L, R) : frequency_(root[y], 0, n - 1, L, R); }
 ...
 private:
  ...
  CEXP void build(u32& x, u32 l, u32 r, u32 pos) {
    if (x = ++cnt; l == r) {
      if (l == pos) ++t[x].w;
      return;
    }
  ...
  CEXP u32 kth_min_(u32 x, u32 y, u32 l, u32 r, u32 k) {
    if (l == r) return l;
  ...
  CEXP u32 kth_max_(u32 x, u32 y, u32 l, u32 r, u32 k) {
    if (l == r) return l;
