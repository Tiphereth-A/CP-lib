...
class mint_s30 {
  ...
  static CEXP u32 MOD = (u32)MOD_, MOD2 = MOD << 1, R2 = -(u64)(MOD) % MOD, R = [] {
  ...
  static_assert(MOD != 1);
  static CEXP u32 redc(u64 x) NE { return u32((x + u64((u32)x * R) * MOD) >> 32); }
 ...
 protected:
  ...
  static CEXP raw_t mod(sint_c auto v) NE { return redc(u64(i32(v % (i32)mod()) + (i32)mod()) * R2); }
  static CEXP raw_t mod(uint_c auto v) NE { return redc(u64(v % mod()) * R2); }
  static CEXP raw_t mod() NE { return MOD; }
  CEXP raw_t val() CNE { return norm(redc(v_)); }
  ...
  CEXP void mul(mint_s30 CR r) NE { v_ = redc(u64(v_) * r.v_); }
};
