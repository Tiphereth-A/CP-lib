...
\[
    \stirlingfirst{n}{k} = \stirlingfirst{n-1}{k-1} + (n-1)\stirlingfirst{n-1}{k}
\]

求同一行: 分治 FFT \(O(n\log ^2 n)\), 或者倍增 \(O(n\log n)\) (每次都是 \(f(x) = g(x) g(x + d)\) 的形式, 可以用 \(g(x)\) 反转之后做一个卷积求出后者).

\[
    \sum_{k = 0}^n \stirlingfirst{n}{k} x^k = \prod_{i = 0}^{n - 1} (x + i)
\]

求同一列: 用一个轮换的指数生成函数做 \(k\) 次幂

\[
    \sum_{n = 0}^\infty \stirlingfirst{n}{k} \frac{x^n}{n!} = \frac{\left(\ln (1 - x)\right)^k}{k!} = \frac{x^k}{k!} \left( \frac{\ln (1 - x)}{x} \right)^k
\]
...
\[
    \stirlingsecond{n}{k} = \stirlingsecond{n-1}{k-1} + k\stirlingsecond{n-1}{k}
\]

求一个: 容斥

\[
    \stirlingsecond{n}{k} = \frac{1}{k!} \sum_{i = 0}^k (-1)^i \binom{k}{i} (k - i)^n = \sum_{i = 0}^k \frac{(-1)^i}{i!} \frac{(k - i)^n}{(k - i)!}
\]
...
\[
    \sum_{n = 0}^\infty \stirlingsecond{n}{k} \frac{x^n}{n!} = \frac{\left(\mathrm{e}^x - 1\right)^k}{k!} = \frac{x^k}{k!} \left( \frac{\mathrm{e}^x - 1}{x} \right)^k
\]

普通生成函数

\[
    \sum_{n = 0}^\infty \stirlingsecond{n}{k} x^n = x^k \left(\prod_{i = 1}^k (1 - i x)\right)^{-1}
\]

\subsubsection{Stirling 反演}

\[
    f(n) = \sum_{k = 0}^n \stirlingsecond{n}{k} g(k) \iff g(n) = \sum_{k = 0}^n (-1)^{n - k} \stirlingfirst{n}{k} f(k)
\]
...
\[
    x^{\overline{n}}=\sum_{k} \stirlingfirst{n}{k} x^k
\]

\[
    x^n=\sum_{k} \stirlingsecond{n}{k} (-1)^{n-k} x^{\overline{k}}
\]

\textbf{下降幂与普通幂的转换}

\[
    x^n=\sum_{k} \stirlingsecond{n}{k} x^{\underline{k}} = \sum_{k} \binom{x}{k} \stirlingsecond{n}{k} k!
\]

\[
    x^{\underline{n}}=\sum_{k} \stirlingfirst{n}{k} (-1)^{n-k} x^k
\]
...
\[
    \sigma_n(x) = \frac{\stirlingfirst{x}{n}}{x(x-1)\dots(x-n)}
\]
...
\[
    \stirlingfirst{n}{n-k} = n^{\underline{k+1}} \sigma_k(n)
\]

\[
    \stirlingsecond{n}{n-k} = (-1)^{k+1} n^{\underline{k+1}} \sigma_k(-(n-k))
\]
...
\[
    k!\stirlingsecond{n}{k} = \sum_{i = 0} ^ {n - 1} \eulerian{n}{i} \binom{i}{n - k}
\]

\subsubsection{二阶 Eulerian Number}
\label{ssec:二阶-eulerian-number}

\(\beulerian{n}{k}\): 每个数都出现两次的多重排列, 并且每个数两次出现之间的数都比它要大. 在此前提下有 \(k\) 个上升的方案数.

\[
    \beulerian{n}{k} = (2n-k-1)\beulerian{n-1}{k-1} + (k+1)\beulerian{n-1}{k}
\]

\[
    \sum_{k = 0} ^ {n - 1} \beulerian{n}{k} = (2n-1)!! = \frac{(2n)^{\underline n}} {2^n}
\]
...
\[
    \stirlingsecond{x}{x-n} = \sum_{k = 0} ^ {n - 1} \beulerian{n}{k} \binom{x + n - k - 1}{2n}
\]

\[
    \stirlingfirst{x}{x-n} = \sum_{k = 0} ^ {n - 1} \beulerian{n}{k} \binom{x + k}{2n}
\]
