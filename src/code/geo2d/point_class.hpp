...
struct Point {
  data_t x, y;
  constexpr Point(data_t x = data_t{}, data_t y = data_t{}):
    x(x), y(y) {}
...
#undef VEC_OP
  ...
  TIFA_LIBS_GEO2D_FUNC_SCALA0__(Point, arg, { return std::atan2(y, x); })
  // result in [0, 2pi)
  TIFA_LIBS_GEO2D_FUNC_SCALA0__(Point, arg2pi, { data_t tmp_ = arg(); return is_negative (tmp_) ? tmp_ + 2 * PI : tmp_; })
  TIFA_LIBS_GEO2D_FUNC_SCALA0__(Point, norm, { return x * x + y * y; })
  TIFA_LIBS_GEO2D_FUNC_SCALA0__(Point, abs, { return sqrt(norm()); })

  constexpr static int QUAD__[9] = {5, 6, 7, 4, 0, 0, 3, 2, 1};
  TIFA_LIBS_GEO2D_FUNC_SCALA0__(Point, quad, { return QUAD__[(sgn(y) + 1) * 3 + sgn(x) + 1]; })

  TIFA_LIBS_GEO2D_FUNC_SELF0__(Point, rot90, { data_t tmp = x; x = -y; y = tmp; return * this; })
  TIFA_LIBS_GEO2D_FUNC_SELF0__(Point, rot180, { x = -x; y = -y; return * this ; })
  TIFA_LIBS_GEO2D_FUNC_SELF0__(Point, rot270, { data_t tmp = y; y = -x; x = tmp; return * this; })
  TIFA_LIBS_GEO2D_FUNC_SELF0__(Point, unit, { return *this /= abs(); })

  TIFA_LIBS_GEO2D_FNUC_SELF1__(Point, rot, data_t, theta, {
    data_t _x = x, _y = y;
