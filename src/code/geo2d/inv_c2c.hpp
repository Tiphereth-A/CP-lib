...
CEXP circle<FP> inv_C2C(circle<FP> CR rc, circle<FP> CR c) {
  const point v = (c.o - rc.o).do_unit(), p1 = inv_P2P(rc, c.o + v * c.r), p2 = inv_P2P(rc, c.o - v * c.r);
...
CEXP circle<FP> inv_C2C(point<FP> CR ro, circle<FP> CR c) {
  const point v = (c.o - ro).do_unit(), p1 = inv_P2P(ro, c.o + v * c.r), p2 = inv_P2P(ro, c.o - v * c.r);
  return {mid_point(p1, p2), dist_PP(p1, p2) / 2};
}
