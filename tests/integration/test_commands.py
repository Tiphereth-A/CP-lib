...
This module tests the integration between different command modules
and their interactions with the configuration and file system.
"""
...
class TestCleanCommand:
    ...
    def test_removes_files(self, sample_dirs: dict[str, Path], caplog: pytest.LogCaptureFixture) -> None:
        """Test that clean command removes files with clean extensions."""
        pdf_out_dir = sample_dirs['_pdf_out']
        test_files = [str(pdf_out_dir / f'test{ext}') for ext in CLEAN_EXT_NAME[:3]]
    ...
    def test_logs_removal(self, sample_dirs: dict[str, Path], caplog: pytest.LogCaptureFixture) -> None:
        """Test that clean command logs file removals."""
        pdf_out_dir = sample_dirs['_pdf_out']
        test_file = str(pdf_out_dir / 'test.log')

        with patch('libs.commands.clean.get_full_filenames') as mock_get_files, \
             patch('libs.commands.clean.os.remove'):
            mock_get_files.return_value = [test_file]
            clean()

        assert 'removed successfully' in caplog.text.lower()

    def test_with_config_integration(
        self, sample_dirs: dict[str, Path], test_config
    ) -> None:
...
class TestFmtCommand:
    ...
    def test_runs_formatting_command(
        self,
        mock_config,
        mock_get_files,
        mock_subprocess,
    ...
    def test_logs_file_count(
        ...
        mock_get_files.return_value = [str(sample_code_file), str(sample_code_file)]
        mock_config.get_code_dir.return_value = 'src'
        mock_config.get_doc_dir.return_value = 'doc'
        mock_config.get_cheatsheet_dir.return_value = 'cheatsheet'
        mock_config.get_usage_dir.return_value = 'usage'
    ...
    @patch('libs.commands.fmt.subprocess.run')
    def test_with_multiple_file_types(
        self,
        mock_subprocess,
...
# =============================================================================
# Typst Content Generation Integration Tests
# =============================================================================
...
class TestTypstContentGeneration:
    ...
    def test_generate_notebook_contents_typst(
        self,
        mock_scandir_file,
        mock_scandir_dir,
        mock_config,
    ...
    def test_generate_cheatsheet_contents_typst(
        self,
        mock_scandir_file,
        mock_config,
        sample_dirs: dict[str, Path]
    ...
    @patch('libs.commands.gen_nb.scandir_dir_merge')
    @patch('libs.commands.gen_nb.scandir_file_merge')
    def test_typst_vs_latex_content_different(
        self,
        mock_scandir_file,
        mock_scandir_dir,
        mock_config,
