...
def verify_codes(src: str, thread_limit: int, time_limit: float, temp_path: str, **kwargs):
    ...
    def _compile(x):
        ...
        _out_file = os.path.join(_dir, os.path.basename(x).replace('.', '_').replace(' ', '-'))
    ...
    run_cases: list[tuple[str, str, str, str]] = []
    for out_f, src_f, samples in out_src_samples.queue:
        if src_f in compile_failed:
            continue
        for inp, expected in samples:
            run_cases.append((out_f, src_f, inp, expected))
    logger.info(f'{len(run_cases)} case(s) found')

    def _run(case: tuple[str, str, str, str]):
        out_f, src_f, inp, expected = case

        def _post(result):
            if result.stdout != expected:
                raise subprocess.CalledProcessError(
                    1, result.args,
                    f'Wrong answer: {src_f}\n'
                    f'    Input:    {inp!r}\n'
