...
namespace uint2str_32_impl_ {

constexpr u32 e2(u64 x, char *s) {
  if (x <= 9) {
    *s = (char)(x | 0x30);
    return 1;
  } else if (x <= 99) {
...
constexpr u32 e4(u64 x, char *s) {
  u64 low = 0, ll = 0;
  u32 digits = 0;
  if (x <= 99) return e2(x, s);
  low = x;
  digits = (low > 999) ? 4 : 3;
  ll = ((low * 5243) >> 19) & 0xFF;
  low -= ll * 100;
...
constexpr u32 uint2str_32(u64 x, char *s) {
  u64 low = 0, ll = 0;
  u32 digits = 0;
  if (x <= 9999) return uint2str_32_impl_::e4(x, s);
  if (x < 100000000) {
    ...
    digits = uint2str_32_impl_::e2(high, s);
    digits += 8;
  }
