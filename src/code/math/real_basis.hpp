...
class basis_R {
 ...
 public:
  ...
  constexpr explicit basis_R(size_t vec_len, FP EPS = 1e-4) : vec_len(vec_len), EPS(EPS) {}

  constexpr vec<FP> &operator[](size_t i) { return base[i]; }
  constexpr vec<FP> operator[](size_t i) const { return base[i]; }
  constexpr vvec<FP> &data() { return base; }
  constexpr vvec<FP> data() const { return base; }

  constexpr size_t rank() const {
    size_t res = 0;
    for (size_t i = 0; i < base.size(); ++i) res += !is_zero(base[i][i]);
    return res;
  }
