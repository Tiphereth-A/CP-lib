...
class fhq_treap {
  ...
  rand::Gen<std::mt19937, std::uniform_int_distribution<int>> gen;
  vec<YYZ> t;
  vec<size_t> sta;
  size_t cnt;
 ...
 public:
  size_t root;
  explicit constexpr fhq_treap(size_t MAX_N) : gen(std::mt19937(), std::uniform_int_distribution<int>()), t(MAX_N + 1), sta(), cnt(0), root(0) {
...
//     size_t sz, ls, rs;
//     tifa_libs::u32 rad;
//     YYZ(T W = e(), size_t SZ = 1, size_t LS = 0, size_t RS = 0) : w(W), sz(SZ), ls(LS), rs(RS), rad(gen()) {}
//   };
//   vec<YYZ> t;
//   vec<size_t> sta;
//   size_t cnt;
