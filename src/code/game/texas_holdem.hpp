...
class th_card {
 ...
 public:
  static CEXP char RANKS[16] = "0123456789TJQKA";
  static CEXP char SUITS[5] = "CDHS";
  explicit CEXP th_card() {}
  CEXP th_card(char rank, char suit) { encode(rank, suit); }
  explicit CEXP th_card(strnv str) : th_card(str[0], str[1]) { assert(str.size() == 2); }
  // Parses a card in a format as "2C"
  // @return: 4 * (rank - 2) + suit  (2 <= rank <= 14)
  CEXP void encode(char rank, char suit) {
    int r = 2, s = 0;
    for (; r < 15; ++r)
  ...
  CEXP int get_rank() const { return (data >> 2) + 2; }
  CEXP int get_suit() const { return data & 3; }
  // @return: {rank_char, suit_char}
  CEXP ptt<char> decode() const { return {RANKS[get_rank()], SUITS[get_suit()]}; }
  ...
  friend std::ostream &operator<<(std::ostream &os, th_card CR card) {
    auto &&_ = card.decode();
...
class th_hand {
 ...
 public:
  explicit CEXP th_hand() : cds(5) {}
  // Set first 5 element as hand
  CEXP void reset(vec<th_card> CR cards) {
    assert(cards.size() >= 5);
    ...
    flt_ (int, r, 2, 15) mpc[cnt[r]] |= 1 << r;
  }
  // Returns the best poker hand with the tie-breaker in [0, 2^20)
  CEXP std::pair<th_category, int> parse() const {
    assert(cds.size() == 5);
  ...
  CEXP th_card &operator[](u32 x) { return cds[x]; }
  CEXP th_card CR operator[](u32 x) const { return cds[x]; }

  friend std::istream &operator>>(std::istream &is, th_hand &hands) {
    flt_ (u32, i, 0, 5) is >> hands.cds[i];
    return is;
  }
  friend std::ostream &operator<<(std::ostream &os, th_hand CR hands) {
    flt_ (u32, i, 0, 4) os << hands.cds[i] << ' ';
    return os << hands.cds[4];
  }

 private:
  // 8. STRAIGHT_FLUSH: highest (5 for A2345)
  static CEXP std::tuple<bool, th_category, int> is_straight_flush(th_hand CR h) {
    int f = 0;
    flt_ (int, s, 0, 4) f |= h.mps[s] & h.mps[s] << 1 & h.mps[s] << 2 & h.mps[s] << 3 & (h.mps[s] << 4 | h.mps[s] >> 14 << 5);
    return {!!f, STRAIGHT_FLUSH, bsr(f)};
  }
  // 7. FOUR_OF_A_KIND: quadruple, other card
  static CEXP std::tuple<bool, th_category, int> is_four_of_a_kind(th_hand CR h) {
  ...
  static CEXP std::tuple<bool, th_category, int> is_full_house(th_hand CR h) {
  ...
  static CEXP std::tuple<bool, th_category, int> is_flush(th_hand CR h) {
    int flush = -1;
  ...
  static CEXP std::tuple<bool, th_category, int> is_straight(th_hand CR h) {
  ...
  static CEXP std::tuple<bool, th_category, int> is_three_of_a_kind(th_hand CR h) {
  ...
  static CEXP std::tuple<bool, th_category, int> is_pair(th_hand CR h) {
  ...
  static CEXP std::tuple<bool, th_category, int> is_high_card(th_hand CR h) { return {true, HIGH_CARD, hbits(h.rka, 5)}; }
  //! The judger of all the categories
  static CEXP std::tuple<bool, th_category, int> (*checks[8])(th_hand CR) = {
      is_straight_flush,
      is_four_of_a_kind,
      is_full_house,
      is_flush,
