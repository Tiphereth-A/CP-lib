...
def cpv_delegate(file_patterns: tuple[str], verify_files: str, merged_result: str, task_count: int, result_dir: str, **kwargs):
    ...
    files_info = {
        k: {'tag': _tag(verify_files_info[k]['verification'][0]),
            'total_elapsed': sum(i['elapsed'] for i in v['verifications']),
            'total_samples': len(v['verifications'][0].get('testcases', []))}
        for k, v in previous_info.items() if k in file_lists} if previous_info else {
        k: _tag(verify_files_info[k])
        for k in file_lists
    }
