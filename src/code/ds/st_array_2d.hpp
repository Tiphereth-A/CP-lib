...
class st_array_2d {
 ...
 public:
  explicit st_array_2d(vvec<T> const &a) : st((u32)bit::log2(std::max(a.size(), a[0].size())) + 1, vvec<T>(a.size(), vec<T>(a[0].size()))) {
    u32 n = a.size(), n2 = a[0].size();
    st[0] = a;
    for (u32 j = 1, m = (u32)bit::log2(std::max(n, n2)) + 1; j < m; ++j)
  ...
  T query(u32 x, u32 y, u32 l) const {
    u32 k = (u32)bit::log2(l + 1), ofs = (1 << k) - 1;
