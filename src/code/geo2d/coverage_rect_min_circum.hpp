#ifndef TIFALIBS_GEO2D_COVERAGE_RECT_MIN_CIRCUM
#define TIFALIBS_GEO2D_COVERAGE_RECT_MIN_CIRCUM
...
CEXP polygon<FP> coverage_rect_min_circum(cT_(cvh<FP>) ch) {
  ...
  return polygon{vec<point<FP>>{ins_LL(li, lp), ins_LL(lp, lr), ins_LL(lr, lq), ins_LL(lq, li)}};
}
