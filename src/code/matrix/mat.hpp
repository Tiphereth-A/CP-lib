...
class matrix {
 ...
 public:
  ...
  constexpr typename vec<T>::reference operator()(usz r, usz c) { return d[r][c]; }
  constexpr typename vec<T>::const_reference operator()(usz r, usz c) const { return d[r][c]; }
  ...
  matrix submat(usz row_l, usz row_r, usz col_l, usz col_r) const {
    ...
    ret.apply_range(0, ret.row(), 0, ret.col(), [this, row_l, row_r](usz i, usz j, T &v) { v = (*this)(i + row_l, j + row_r); });
    return ret;
  }
  ...
  constexpr matrix &operator+=(const matrix &r) {
    ...
    apply_range(0, row(), 0, col(), [&r](usz i, usz j, T &val) { val += r(i, j); });
    return *this;
  }
  constexpr friend matrix operator-(matrix l, const matrix &r) { return l -= r; }
  constexpr matrix &operator-=(const matrix &r) {
  ...
  vec<T> lproj(vec<T> const &x) const {
    ...
    assert(r_ == x.size());
    vec<T> ret(c_);
    for (usz i = 0; i < c_; ++i) ret[i] = std::transform_reduce(d[i].begin(), d[i].end(), x.begin(), T{});
    return ret;
  }
};
