...
requires requires(ostree_node_t<tag_t, K> *&root, ostree_node_t<tag_t, K> n, tag_t tag, bool dir) {
  n.fa->ch[0]->ch[1]->data;
  n.sz;
...
struct ostree : tag_t {
  ...
  CEXP const_pointer erase(pointer p) {
    ...
    return tag_t::modify_size(p, -(i32)tag_t::count(p)), tag_t::erase_branch_leaf(root, p), alloc.deallocate(p, 1), result;
  }
