...
struct rational {
  ...
  CEXP rational &operator+=(T x) { return num += x * den, *this; }
  CEXP rational &operator-=(T x) { return num -= x * den, *this; }
  CEXP rational &operator*=(T x) {
    const T g = (T)gcd(x, den);
    return num *= x / g, den /= g, *this;
  }
  CEXP rational &operator/=(T x) {
    assert(x);
  ...
  CEXP rational &operator+=(rational CR x) {
    T g = (T)gcd(den, x.den);
    return num = num * (x.den / g) + x.num * (den /= g), g = (T)gcd(num, g), num /= g, den *= x.den / g, *this;
  }
  CEXP rational &operator-=(rational CR x) {
    T g = (T)gcd(den, x.den);
    return num = num * (x.den / g) - x.num * (den /= g), g = (T)gcd(num, g), num /= g, den *= x.den / g, *this;
  ...
  CEXP rational &operator*=(rational CR x) {
    ...
    return num = num / g1 * (x.num / g2), den = den / g2 * (x.den / g1), *this;
  }
  CEXP rational &operator/=(rational CR x) {
  ...
  CEXP rational &operator++() { return num += den, *this; }
  CEXP rational &operator--() { return num -= den, *this; }
