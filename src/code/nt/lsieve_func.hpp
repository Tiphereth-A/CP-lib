...
struct ls_mpf_tag {
 ...
 protected:
  CEXPE ls_mpf_tag(u32 n) : mpf(n) { (n > 1) && (mpf[1] = 1); }
...
struct ls_phi_tag {
 ...
 protected:
  CEXPE ls_phi_tag(u32 n) : phi(n) { (n > 1) && (phi[1] = 1); }
...
struct ls_tau_tag {
 ...
 protected:
  vecu pc;
  CEXPE ls_tau_tag(u32 n) : tau(n), pc(n, 1) { (n > 1) && (tau[1] = 1); }
...
struct ls_mu_tag {
 ...
 protected:
  CEXPE ls_mu_tag(u32 n) : mu(n) { (n > 1) && (mu[1] = 1); }
...
struct ls_sigma_tag {
 ...
 protected:
  vecuu pw;
  CEXPE ls_sigma_tag(u32 n) : sigma(n), pw(n) { (n > 1) && (sigma[1] = 1); }
