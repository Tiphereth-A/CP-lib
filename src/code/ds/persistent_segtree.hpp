...
class persistent_segtree {
  ...
  size_t kth_min_(i32 x, i32 y, size_t l, size_t r, size_t k) {
    if (l == r) return l;
  ...
  size_t kth_max_(i32 x, i32 y, size_t l, size_t r, size_t k) {
    if (l == r) return l;
  ...
  size_t frequency_(i32 x, i32 y, size_t l, size_t r, size_t L, size_t R) {
 ...
 public:
  persistent_segtree(vec<size_t> const& a, size_t N) : n(N), cnt(0), t(a.size() * 24, YYZ()), root(a.size(), -1) {
    build(root[0], 0, n - 1, a[0]);
    for (size_t i = 1; i < a.size(); ++i) add_(root[i - 1], root[i], 0, n - 1, a[i]);
  }
  void add(i32 old_x, i32 x, size_t pos) { add_(root[old_x], root[x], 0, n - 1, pos); }
  size_t kth_min(i32 x, i32 y, size_t k) { return x ? kth_min_(root[x - 1], root[y], 0, n - 1, k) : kth_min_(root[y], 0, n - 1, k); }
  size_t kth_max(i32 x, i32 y, size_t k) { return x ? kth_max_(root[x - 1], root[y], 0, n - 1, k) : kth_max_(root[y], 0, n - 1, k); }
  size_t frequency(i32 x, i32 y, size_t pos) { return x ? frequency_(root[x - 1], root[y], 0, n - 1, pos, pos) : frequency_(root[y], 0, n - 1, pos, pos); }
  size_t frequency(i32 x, i32 y, size_t L, size_t R) { return x ? frequency_(root[x - 1], root[y], 0, n - 1, L, R) : frequency_(root[y], 0, n - 1, L, R); }
};
