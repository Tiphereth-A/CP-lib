...
template <class mint>
vec<mint> conv_ntt(vec<mint> const &l, vec<mint> const &r) { return conv_ntt(l, r, l.size() + r.size() - 1); }
...
template <class mint, class T = u64>
vec<mint> conv_ntt(vec<T> const &l, vec<T> const &r) { return conv_ntt<mint, T>(l, r, l.size() + r.size() - 1); }
