...
template <class poly>
constexpr poly polysp_pow(poly const& p, u64 y, vec<u64> const& inv, u32 n = 0) {
  if (!n) n = p.size();
  if (!y) return poly{1}.pre(n);
  ...
  g[0] = qpow(p[0], y);
  auto _ = p[0].inv();
  for (u64 k = 1, y_ = y % poly::value_type::mod(); k < n; ++k) {
    for (auto& [j, pj] : ps) {
      if (k < j) break;
...
template <class poly>
constexpr poly polysp_pow(poly const& p, u64 y, u32 n = 0) {
  if (!n) n = p.size();
  return polysp_pow(p, y, gen_inv(n, poly::value_type::mod()), n);
}
