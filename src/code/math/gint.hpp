...
class gint {
 ...
 public:
  ...
  friend auto &operator>>(tifa_libs::istream_c auto &is, gint &x) NE { return is >> x.r_ >> x.i_; }
  friend auto &operator<<(tifa_libs::ostream_c auto &os, gint CR x) NE { return os << x.real() << ' ' << x.imag(); }
};
