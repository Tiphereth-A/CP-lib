...
class segbeats_ca_msh {
 ...
 public:
  explicit CEXP segbeats_ca_msh(vec<T> CR a) : n(a.size()), t(a.size() * 4), tag(a.size() * 4), INF(std::numeric_limits<T>::max()) { build(a, 1, 0, n - 1); }
  explicit CEXP segbeats_ca_msh(u32 N) : n(N), t(N * 4), tag(N * 4), INF(std::numeric_limits<T>::max()) { build(vec<T>(n, 0), 1, 0, n - 1); }
 ...
 private:
  CEXP void pushup(u32 x) {
    t[x].sum = t[x * 2].sum + t[x * 2 + 1].sum, t[x].hismax = max(t[x * 2].hismax, t[x * 2 + 1].hismax);
    if (t[x * 2].min == t[x * 2 + 1].min) t[x].min = t[x * 2].min, t[x].min2 = min(t[x * 2].min2, t[x * 2 + 1].min2), t[x].cmin = t[x * 2].cmin + t[x * 2 + 1].cmin;
    else if (t[x * 2].min < t[x * 2 + 1].min) t[x].min = t[x * 2].min, t[x].min2 = min(t[x * 2].min2, t[x * 2 + 1].min), t[x].cmin = t[x * 2].cmin;
    else t[x].min = t[x * 2 + 1].min, t[x].min2 = min(t[x * 2].min, t[x * 2 + 1].min2), t[x].cmin = t[x * 2 + 1].cmin;
    if (t[x * 2].max == t[x * 2 + 1].max) t[x].max = t[x * 2].max, t[x].max2 = max(t[x * 2].max2, t[x * 2 + 1].max2), t[x].cmax = t[x * 2].cmax + t[x * 2 + 1].cmax;
    else if (t[x * 2].max > t[x * 2 + 1].max) t[x].max = t[x * 2].max, t[x].max2 = max(t[x * 2].max2, t[x * 2 + 1].max), t[x].cmax = t[x * 2].cmax;
    else t[x].max = t[x * 2 + 1].max, t[x].max2 = max(t[x * 2].max, t[x * 2 + 1].max2), t[x].cmax = t[x * 2 + 1].cmax;
  }
  CEXP void all_update(u32 x, u32 l, u32 r, T add1, T add2, T add3, T maxadd1, T maxadd2, T maxadd3) {
    ...
    t[x].hismax = max(t[x].hismax, t[x].max + maxadd2), tag[x].maxadd1 = max(tag[x].maxadd1, tag[x].add1 + maxadd1), tag[x].maxadd2 = max(tag[x].maxadd2, tag[x].add2 + maxadd2), tag[x].maxadd3 = max(tag[x].maxadd3, tag[x].add3 + maxadd3);
    ...
    t[x].min += add1, t[x].max += add2, tag[x].add1 += add1, tag[x].add2 += add2, tag[x].add3 += add3;
  }
  ...
  CEXP void build(vec<T> CR a, u32 x, u32 l, u32 r) {
    ...
    u32 mid = l + (r - l) / 2;
    build(a, x * 2, l, mid), build(a, x * 2 + 1, mid + 1, r), pushup(x);
  }
};
